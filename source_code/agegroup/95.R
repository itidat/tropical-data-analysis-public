...
library(sqldf)
mydb_tf <- as.data.frame(sqldf('SELECT eu, cluster, age AS age_group, COUNT(instance_id_res) AS residents,
                 SUM(CASE WHEN (left_eye_tf=1 OR right_eye_tf=1) THEN 1 ELSE 0 END) AS tf_pos
                 FROM clean
                 WHERE cluster_complete>0 AND age BETWEEN 1 AND 9 AND examined = 1 and left_eye_tf is not null
                 GROUP BY eu, cluster, age
                 ORDER BY eu, cluster, age'))

mydb_ti <- as.data.frame(sqldf('SELECT eu, cluster, age AS age_group, COUNT(instance_id_res) AS residents,
...
mydb_tt <- as.data.frame(sqldf('SELECT eu, cluster, (CASE
                 ...
                 SUM(CASE WHEN tt_old_unmanaged=1 THEN 1 ELSE 0 END) as tt_old_unmanaged
                 FROM clean
                 WHERE cluster_complete>0 AND examined = 1 AND age >= 15 and left_eye_tf is not null
