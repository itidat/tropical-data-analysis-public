## Tropical Data standard analyses - Bootstrapping EU mean and 95% CI based on adjusted TT v1 cluster prevalence
## v1 methodology: TT is upper and/or lower lid trichiasis + scarring
...
if(exists("dataset")) {
  rm(dataset)
  dataset <- subset(ttprev_cluster2, ttprev_cluster2$eu %in% tt1EU)
} else {
  dataset <- subset(ttprev_cluster2, ttprev_cluster2$eu %in% tt1EU)
}
...
dataset$cluster_prev <- dataset$tt1
...
bootResult_tt1 <- data.frame(eu=character(), bootmean=numeric(), se=numeric(), tt1_95_low=numeric(), tt1_95_upp=numeric(), stringsAsFactors=FALSE)
...
# write result to data frame
  ...
  bootResult_tt1[i,] <- c(eu_temp, m, se, ci_lower, ci_upper)
...
if(exists("dataset")) {
  rm(dataset)
  dataset <- subset(ttprev_cluster2, ttprev_cluster2$eu %in% tt1_unmanagedEU)
} else {
  dataset <- subset(ttprev_cluster2, ttprev_cluster2$eu %in% tt1_unmanagedEU)
}
...
dataset$eu <- as.factor(dataset$eu)
dataset$cluster <- as.factor(dataset$cluster)
dataset$cluster_prev <- dataset$tt1_unmanaged
...
# create empty data frame for results
bootResult_tt1_unman <- data.frame(eu=character(), bootmean=numeric(), se=numeric(), tt1_unmanaged_95_low=numeric(), 
                                     tt1_unmanaged_95_upp=numeric(), stringsAsFactors=FALSE)
...
# write result to data frame
  eu_temp <- as.character(data2$eu[1])
  bootResult_tt1_unman[i,] <- c(eu_temp, m, se, ci_lower, ci_upper)
