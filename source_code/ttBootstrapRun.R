...
# Next it creates the dataset, which will be composed only of EUs where there is 1+ cases of Trichiasis + TS
if(exists("dataset")) {
  rm(dataset)
  dataset <- subset(ttprev_cluster2, ttprev_cluster2$eu %in% ttEU)
} else {
  dataset <- subset(ttprev_cluster2, ttprev_cluster2$eu %in% ttEU)
}
...
dataset$cluster_prev <- dataset$tt_true
...
for (i in 1:nlevels(dataset$eu)) {
  ...
  bootResult_tt[i,] <- c(eu_temp, m, se, ci_lower, ci_upper)
...
###UNMANAGED Trichiasis + TS
#Load, transform data

# IF statement removes the dataset if already generated for another bootstrapping exercise
# Next it creates the dataset, which will be composed only of EUs where there is 1+ cases of unmanaged Trichiasis + TS
ttprev_cluster2 <- ttprev_cluster


if(exists("dataset")) {
  rm(dataset)
  dataset <- subset(ttprev_cluster2, ttprev_cluster2$eu %in% tt_unmanagedEU)
} else {
  dataset <- subset(ttprev_cluster2, ttprev_cluster2$eu %in% tt_unmanagedEU)
}
...
dataset$eu <- as.factor(dataset$eu)
dataset$cluster <- as.factor(dataset$cluster)
dataset$cluster_prev <- dataset$tt_unmanaged
...
#create empty data frame for results
bootResult_tt_unman <- data.frame(eu=character(), bootmean=numeric(), se=numeric(), tt_unmanaged_95_low=numeric(), 
                                     tt_unmanaged_95_upp=numeric(), stringsAsFactors=FALSE)
...
for (i in 1:nlevels(dataset$eu)) {
  ...
  #write result to data frame
  eu_temp <- as.character(data2$eu[1])
  bootResult_tt_unman[i,] <- c(eu_temp, m, se, ci_lower, ci_upper)
